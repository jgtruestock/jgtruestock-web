...
// 已完成的 26 支（跳過）
const DONE = new Set([
  'GOOGL','ASML','CRWD','MP','HOOD','COIN','NFLX','ORCL','ANET','VRTX',
  'NTLA','BEAM','NOC','OKLO','SMR','XYL','PNR','BEPC','CACI','MSTR',
  'QCOM','CRDO','BABA','ARES','META','AAPL'
]);
...
async function main() {
  ...
  const remaining = all.filter(c => !DONE.has(c.symbol));
  console.log(`繼續批次更新剩餘 ${remaining.length} 支股票 Block C...\n`);
  console.log(`跳過已完成：${[...DONE].join(', ')}\n`);
  ...
  for (let i = 0; i < remaining.length; i++) {
    ...
    console.log(`[${i + 1}/${remaining.length}] ${symbol}...`);
  ...
  console.log(`\n全部總計：前批 26 支 + 本批 ${success} 支 = ${26 + success} 支`);
